...
class Batch extends CI_Controller
{
    ...
    public function newapplication()
    {
        ...
        $_SESSION['s_menu'] = 'NewApplication';
...
/*			    'id' => abs((int) filter_var($this->input->post('txtIdDirect'), FILTER_SANITIZE_NUMBER_INT)),*/
               /* 'generated_id' => $this->input->post('txtIdDirect'),*/
                'nes_id' => $this->input->post('txtNesId'),
                'batch' => $this->input->post('txtBatch'),
        ...
        if ($this->db->insert('application', $ipplicationData)) {
            $genID = $this->input->post('abbr').'/'.$this->db->insert_id().'/'.$this->input->post('year');
            $this->db->where('id', $this->db->insert_id());
            if ($this->db->update('application', array('generated_id' => $genID))) {
                echo $genID;
                //echo $this->input->post('txtIdDirect');
            }
        } else {
            echo "error";
        }
    ...
    public function studentenroll()
    {
        ...
        $data['batches'] = $this->db->query('select b.id, c.course, b.course_id, b.year_span, c.abbr from batch_master b, course_master c where c.id = b.course_id order by b.year_span desc');
        $_SESSION['m_menu']='Madmission';
        $_SESSION['s_menu'] = 'StudentEnroll';

        if ($this->input->get('id', true) != '') {
            $data['id'] = $this->input->get('id', true);
        }
        $this->load->view('student_enroll', $data);
    }

    public function studentenroll_()
    {
        $this->load->database();
        $ipplicationData = array(
                'enrolment_receipt' => $this->input->post('txtReceipt'),
                'amount' => $this->input->post('txtAmount'),
                'enrolment_date' => $this->makedate($this->input->post('txtDate')),
                'status' => 'Student',
                'enrolemnt_year' =>  $this->input->post('selYear')
            );
        $this->db->where('id', $this->input->post('txtAppId'));

        if ($this->db->update('application', $ipplicationData)) {
            echo $this->input->post('txtStudName');
        } else {
            echo "error";
        }
    }

    public function fixprefix()
    {
        $this->load->view('fixprefix');
    }
